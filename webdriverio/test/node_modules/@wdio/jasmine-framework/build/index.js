...
class JasmineAdapter {
  ...
  async run() {
    ...
    INTERFACES['bdd'].forEach(fnName => (0, _config.runTestInFiberContext)(['it', 'fit', 'xit'], this.config.beforeHook, this.config.afterHook, fnName));
    ...
    await (0, _config.executeHooksWithArgs)(this.config.after, [result, this.capabilities, this.specs]);
    return result;
  }
  ...
  wrapHook(hookName) {
    return done => (0, _config.executeHooksWithArgs)(this.config[hookName], this.prepareMessage(hookName)).then(() => done(), e => {
